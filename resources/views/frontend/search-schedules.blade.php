...
<div class="row g-3">
    <div class="col-lg-4"> 
        <form id="search-form"  action="{{ route('schedules.search') }}" method="POST" class="card p-3">
            ...
            <div class="form-floating mb-3">
                <select class="form-control rounded-3" id="destination_terminal_id" name="destination_terminal_id" required>
            ...
            <div class="form-floating mb-3">
                <input type="date" class="form-control rounded-3" id="departure_date" name="departure_date" required>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response?.schedules?.length > 0) {
                            ...
                            const html = `
                                <div class="schedule-item my-2 p-3 border rounded">
                                    ...
                                    <div class="action-button">
                                        <button class="btn btn-lg rounded-3 btn-primary">Book Now</button>
                                    </div>
                                </div>`;
                        ...
                        $('#schedules-results').append('<p>No schedules found for the selected date.</p>');
                    }
                },
                error: function(xhr, status, error) {
                    // Handle any errors here
                    $('#schedules-results').empty();
                    $('#schedules-results').append('<p>An error occurred while searching for schedules.</p>');
                }
            });
        });
    });
